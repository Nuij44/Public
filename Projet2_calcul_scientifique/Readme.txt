...
*******************************************
Le dossier test des méthodes contient un petit code permettant de tester et comparer la méthode de décomposition de Adomain et celle des noyaux itérés sur un cas simple et soluble analytiquement détaillé dans le rapport.
...
-"fredholm.c" contient les fonctions appliquant la méthode de décomposition de Adomain nomée adomain() et la méthode des noyaux itérés nommée noyaux_iteres().

-"main" contient l'algorithme principal qui permet de choisir la finesse du maillage de Omega avec N_L, le alpha choisit avec alpha_op et la limite du nombre de terme calculés dans les séries avec itmax_serie. De plus, il y des flags ligne 37 et 38 pour le choix de la méthode de résolution de l'intégrale de Fredholm de seconde espèce puis ligne 39 il y a le nombre de d'itération faites par la méthode choisie.
